...
Type objective_function<Type>::operator() ()
{
  ...
  // ======== Parameters =================================
    ...
    PARAMETER(Sslope); // Selectivity slope parameter
    PARAMETER(S50); // age at 50% selectivity
    PARAMETER(dome); // dome slope
    PARAMETER(CV_c); // CV of catch
    PARAMETER(CV_l); // CV of average length for each sample
  ...
  for(int a=0;a<=AgeMax;a++){
    S_a(a) = exp(dome*Sslope*(S50-Type(a)))/(1+exp(-Sslope*(Type(a) - S50)));
  }
  ...
  for(int t=1;t<n_t;t++){
    ...
    if(RecType==0 | RecType==2) R_t(t) = exp(beta) * exp(Nu_input(t) - RecDev_biasadj(t)*pow(sigma_R,2)/Type(2));
      // Beverton-Holt
    if(RecType==1) R_t(t) = ((4*h*exp(beta)*SB_t(t-1)) / (SB0*(1-h)+SB_t(t-1)*(5*h-1))) * exp(Nu_input(t) - RecDev_biasadj(t)*pow(sigma_R,2)/Type(2));
    ...
    for(int t=0;t<n_t;t++){
      ...
      for(int i=0;i<n_i;i++){
        if(I_yrs(i)==T_yrs(t)){
          ...
          if(rel_i==0) log_pI_t(t) += dnorm( I_t(i), I_t_hat(t), I_t_hat(t)*CV_c, true);
          if(rel_i==1) log_pI_t(t) += dnorm( I_t(i), I_t_hat_rel(t), I_t_hat_rel(t)*CV_c, true);
          if(rel_i==2){
            log_pI_t(t) += dnorm( I_t(i), I_t_hat(t), I_t_hat(t)*CV_c, true);
  ...
  if(n_c>0){
    for(int t=0;t<n_t;t++){  
      ...
      for(int c=0;c<n_c;c++){
        if(C_yrs(c)==T_yrs(t)){
          // probability of index at that sample
          if(rel_c==0) log_pC_t(t) += dnorm( C_t(c), C_t_hat(t), C_t_hat(t)*CV_c, true);
          if(rel_c==1) log_pC_t(t) += dnorm( C_t(c), C_t_hat_rel(t), C_t_hat_rel(t)*CV_c, true);
          if(rel_c==2){
            log_pC_t(t) += dnorm( C_t(c), C_t_hat(t), C_t_hat(t)*CV_c, true);
            log_pC_t(t) += dnorm( C_t(c), C_t_hat_rel(t), C_t_hat_rel(t)*CV_c, true);
          }
        }
      }
    }
